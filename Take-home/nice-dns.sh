...
function install_dnsserver {
  ...
  # interface eth1 toevoegen, dns server mag enkel luisteren naar localhost en eth1
  sudo sed -i '/^interface=lo/a interface=eth1' /etc/dnsmasq.conf 
...
function fancy_lookup {
  ...
  local resource_record
  resource_record=$(nslookup "${URL}"."${DOMAIN}" "${Serv}")
